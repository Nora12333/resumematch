...
export default function App() {
  ...
  const handleAnalyze = async () => {
    if (!resumeText.trim() || !jdText.trim()) { setError(t.emptyInput); return; }
    try {
      setError(""); setAnalysisResult(null); setGeneratedResult(null);
      setCurrentStep(1); setAnalyzing(true);
      const { data } = await axios.post(`${API_BASE}/api/analyze`, { resume_text: resumeText, jd_text: jdText });
      setAnalysisResult(data); setCurrentStep(2);
    } catch { setError(t.apiError); } finally { setAnalyzing(false); }
  };
  const handleGenerate = async () => {
    ...
    try {
      ...
      const { data } = await axios.post(`${API_BASE}/api/generate`, {
        resume_text: resumeText, jd_text: jdText, gaps: analysisResult.gaps, mode,
      });
      setGeneratedResult(data); setCurrentStep(3);
    } catch { setError(t.apiError); } finally { setGenerating(false); }
  };
  return (
    <main className="mx-auto min-h-screen max-w-6xl px-4 py-8">
      ...
      <InputStep t={t} resumeText={resumeText} jdText={jdText}
        setResumeText={setResumeText} setJdText={setJdText}
      ...
      {(currentStep >= 3 || generatedResult) && (
        <div className="mt-5">
          <GenerateStep t={t} mode={mode} setMode={setMode}
            generatedResult={generatedResult} onGenerate={handleGenerate} loading={generating}
            resumeText={resumeText} jdText={jdText}
            gaps={analysisResult?.gaps || []} apiBase={API_BASE} />
        </div>
      )}
    </main>
  );
